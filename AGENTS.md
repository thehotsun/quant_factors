...
| 9 | 数据下载 + 定时刷新都要加 | 新因子没数据可算 | `download_history.py` + `core/data_refresh.py` |
...
在 `core/data_refresh.py` 的 `daily_data_refresh()` / `daily_data_refresh_foreign()` 中添加数据刷新。
...
5. `core/factor_runner.py` / `core/data_refresh.py` / `server.py` — 3 处修改（import + 定时刷新 + 综合路由）
...
| ☐ `core/data_refresh.py` 添加定时刷新 | `core/data_refresh.py` |
