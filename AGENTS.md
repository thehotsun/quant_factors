...
| 8 | 导入到 `server.py._ensure_imported()` | 装饰器不执行，因子不注册 | `server.py` |
...
**第 4 步：注册导入 + 定时刷新**（`server.py`）

在 `_ensure_imported()` 中添加 `import factors.xxx.xxx`。
在 `_daily_data_refresh()` 的 `tasks` 列表中添加数据刷新。
...
5. `server.py` — 3 处修改（import + 定时刷新 + 综合路由）
...
| ☐ `server.py._ensure_imported()` 添加 import | `server.py` |
